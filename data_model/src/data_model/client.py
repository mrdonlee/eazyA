...
class Client():
    ...
    def uploadDataset(self, dataset_id: str, storage_url: str, price: int):
        ...
        purchase_tx = self.contract.functions.purchaseAccess(dataset_id).build_transaction({
            ...
            'gas': 2000000,
            'gasPrice': web3.to_wei('50', 'gwei'),
            # This gets a fresh nonce
            'nonce': web3.eth.get_transaction_count(self.account.address),
            'value': price  # Send the price as value for the purchase
        })

        # Sign the purchase transaction
        signed_purchase_tx = web3.eth.account.sign_transaction(
            purchase_tx, self.pk)

        # Send the purchase transaction
        purchase_hash = web3.eth.send_raw_transaction(
            signed_purchase_tx.raw_transaction)
        ...
        # Verify access
        has_access = self.contract.functions.hasAccess(
            dataset_id).call({'from': self.account.address})
        print(f"Access granted: {has_access}")

        return has_access

    def downloadDataset(self, dataset_id: str) -> Optional[str]:
        """
        ...
        df = convert_to_dataframe(storage_url)
